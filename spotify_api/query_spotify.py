...
# Safely makes an api call, retrying at the correct times
...
def get_album_features(spotify, album_id):
    ...
    # Replace None, with correctly sized dict
    album_features = [dict.fromkeys(their_keys) if x is None else x for x in
                      album_features]
...
def main():
    ...
    for o, a in opts:
        ...
        if o in ("-i", "--index"):
            start_index = int(a)
            count = int(a)
            print(df_pitchfork.loc[start_index:start_index].apply(
                process_row, axis=1))
    ...
    try:
        ...
        for i in range(start_index, df_pitchfork.shape[0], n):
            df = df_pitchfork.loc[i:i+n]
            df_chunk = df.apply(process_row, axis=1)
            df_chunk.to_csv(outfile_csv, header=False, mode='a')
